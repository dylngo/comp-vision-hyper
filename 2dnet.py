...
PARAM_DATASET = "M"  # UP,IN,SV,KSC,AB,M
PARAM_NETWORK = "pyresnet" # resnet, pyresnet
PARAM_EPOCH = 200
PARAM_ITER = 3
PATCH_SIZE = 0
PARAM_VAL = 0.6
...
def load_dataset(Dataset, split=0.9):
    ...
    if Dataset == 'AB':
        SV = sio.loadmat(data_path + 'Albedo16.mat')
        gt_SV = sio.loadmat(data_path + 'Albedo16_gt.mat')
        data_hsi = SV['Albedo16']
        gt_hsi = gt_SV['Albedo16_gt']
        K = 8
        TOTAL_SIZE = 2000
        VALIDATION_SPLIT = split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
    ...
    if Dataset == 'M':
        SV = sio.loadmat(data_path + 'Materials.mat')
        gt_SV = sio.loadmat(data_path + 'Materials_gt.mat')
        data_hsi = SV['Materials']
        gt_hsi = gt_SV['Materials_gt']
        K = 16
        TOTAL_SIZE = 2000
        VALIDATION_SPLIT = split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
    ...
    def __init__(self,
        ...
        if zero_init_residual:
            for m in self.modules():
                ...
                elif isinstance(m, RNBasicBlock):
...
class PRNBasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(PRNBasicBlock, self).__init__()
        self.conv1 = nn.Conv2d(
            inplanes,
            planes,
            kernel_size=3,
...
def PyResNet34(pretrained=None, **kwargs):
    ...
    if pretrained:
        raise NotImplementedError()
    model = PyResNet(PRNBasicBlock, [3, 4, 6, 3], **kwargs)
    return model

#####################################################################

if PARAM_NETWORK=="resnet":
    model = ResNet34(
        in_shape=(BAND, img_rows, img_cols), num_classes=CLASSES_NUM).cuda()

if PARAM_NETWORK=="pyresnet":
    model = PyResNet34(
        in_shape=(BAND, img_rows, img_cols), num_classes=CLASSES_NUM).cuda()


input_data = (BAND, img_rows, img_cols)
verbose = 1

summary(model, input_data, verbose)
...
# this sampling method discards part of the ground truth which is why it doesn't add up to the number of pixels
# it discards the part corresponding to the actual ground
# TODO: adapt so that variable total test size can be used
...
for index_iter in range(ITER):
    ...
    with torch.no_grad():
        for X, y in test_iter:
            ...
            pred_matrix.extend(np.array(net(X).cpu()))
    toc2 = time.time()
    collections.Counter(pred_test)
    gt_test = gt[test_indices] - 1

    test_acc = metrics.accuracy_score(gt_test[:-VAL_SIZE], pred_test)
    confusion_matrix = metrics.confusion_matrix(pred_test, gt_test[:-VAL_SIZE])
    each_acc, average_acc = record.aa_and_each_accuracy(confusion_matrix)
    kappa = metrics.cohen_kappa_score(pred_test, gt_test[:-VAL_SIZE])

    if PARAM_NETWORK=="resnet":
        PATH = "./models/" + 'Resnet34' + str(round(test_acc, 3)) + '.pt'
    if PARAM_NETWORK=="pyresnet":
        PATH = "./models/" + 'PyResnet34' + str(round(test_acc, 3)) + '.pt'
    ...
    if ELEMENT_ACC.shape[1] > 3:
        test_acc_top_3 = metrics.top_k_accuracy_score(gt_test[:-VAL_SIZE],pred_matrix,k=3)
        OA3.append(test_acc_top_3)

    if ELEMENT_ACC.shape[1] > 5:
        test_acc_top_5 = metrics.top_k_accuracy_score(gt_test[:-VAL_SIZE],pred_matrix,k=5)
        OA5.append(test_acc_top_5)

# # Map, Records
print("--------" + " Training Finished-----------")

if PARAM_NETWORK == "resnet":
    PATH = "./report/" + 'Resnet34patch_' + str(img_rows) + '_' + Dataset + 'split' + str(VALIDATION_SPLIT) + 'lr' + str(lr) + PARAM_OPTIM + '.txt'
if PARAM_NETWORK == "pyresnet":
    PATH = "./report/" + 'PyResnet34patch_' + str(img_rows) + '_' + Dataset + 'split' + str(VALIDATION_SPLIT) + 'lr' + str(lr) + PARAM_OPTIM + '.txt'

record.record_output(OA, AA, KAPPA, ELEMENT_ACC, TRAINING_TIME, TESTING_TIME, PATH, OA3, OA5)

# uncomment if you want the maps
# PATH = "./classification_maps/" + 'PyResnet34patch_' + str(img_rows) + '_' + Dataset + 'split' + str(VALIDATION_SPLIT) + 'lr' + str(lr) + PARAM_OPTIM
# Utils.generate_png(all_iter, net, gt_hsi, Dataset, device, total_indices, PATH)
